...
char host_url[HOST_URL_LENGTH] = "https://www.iringer.kr/";
...
//  bool pressed;
//};
//Button SW_ON = {32, 0, false};
Button SW_ON = {33, 0, false};//up
void IRAM_ATTR isr5()
{
...
//SOUND OFF
  ...
  pinMode(SW_ON.PIN, INPUT_PULLUP);
  attachInterrupt(SW_ON.PIN, isr5, FALLING);
...
byte get_list()
{
  ...
  int httpCode = post_graphql_query(body, "query {  aps { ssid pw } }");
  ...
  if (httpCode == 200)
  {
    ...
    if (root.success() && root["data"]["aps"].is<JsonObject>()) {
      String aps = root["data"]["aps"];
      Serial.println(str);
      // TODO write aps
      x = 1;
    } else
...
byte get_info()
{
  ...
  if (httpCode == 200)
  {
    ...
    if (root["data"]["v1Device"].is<JsonObject>()) {
      String r_vol_max = root["data"]["v1Device"]["r_volume_max"];
      String r_vol_now = root["data"]["v1Device"]["r_volume_now"];
      String r_adrop = root["data"]["v1Device"]["r_adrop"];
      String ordered_gtt = root["data"]["v1Device"]["ordered_gtt"];
      String min_gtt = root["data"]["v1Device"]["min_gtt"];
      String max_gtt = root["data"]["v1Device"]["max_gtt"];
      g_RingerData.r_volume_max = r_vol_max.toInt();
